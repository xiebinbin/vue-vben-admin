<template>
  <PageWrapper>
    <BasicTable :loading="loading" :dataSource="items" @register="registerTable">
      ...
      <template #toolbar>
        <a-button type="primary" @click="handleAdd">添加分類</a-button>
      </template>
    </BasicTable>
  </PageWrapper>
</template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const loadData = async () => {
        ...
        items.value = data?.data.expert_system_staff_categories;
      };
      onMounted(async () => {
        loadData();
      });
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.removeStaffCategoryGQL,
            variables: {
              id: record.id,
            },
          })
      ...
      const handleEdit = (record: Recordable) => {
        instance?.proxy?.$router.push({
          path: '/expert-system/staff-category/edit',
          query: { id: record.id },
        });
      };
      const handleAdd = () => {
