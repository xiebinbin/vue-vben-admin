...
<script lang="ts">
  ...
  const pageTitle = ref<string>('专家分类');
      ...
      if (!!itemId.value) {
        ...
        onMounted(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.staffCategoryGQL,
            variables: {
              id: itemId.value,
            },
          });
          const info = data?.data?.expert_system_staff_category;
          methods.setFieldsValue({ name: info.name });
        });
        pageTitle.value = '編輯專家分類';
      } else {
        pageTitle.value = '新增專家分類';
      }
      return {
        schemas,
        handleReset,
        handleSubmit: (values: any) => {
          if (!!itemId.value) {
            ...
            instance?.proxy?.$apollo
              ?.mutate({
                mutation: gql.updateStaffCategoryGQL,
                variables: {
                  id: itemId.value,
                  input: {
                    name: values.name,
            ...
            instance?.proxy?.$apollo
              ?.mutate({
                mutation: gql.addStaffCategoryGQL,
                variables: {
                  input: {
                    name: values.name,
                  },
