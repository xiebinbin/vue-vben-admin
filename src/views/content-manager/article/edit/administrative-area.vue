...
<script lang="ts">
  ...
  const pageTitle = ref<string>('行政区域管理');
      ...
      if (!!itemId.value) {
        ...
        onMounted(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.articleGQL,
            variables: {
              id: itemId.value,
            },
          });
          const info = data?.data?.article;
          methods.setFieldsValue({
            title: info.title,
            desc: info.desc,
            sort: info.sort,
          ...
          formData.recommends = info.recommends;
        });
        pageTitle.value = '编辑行政区域';
      } else {
        pageTitle.value = '新增行政区域';
      }
      return {
        schemas,
        handleReset,
