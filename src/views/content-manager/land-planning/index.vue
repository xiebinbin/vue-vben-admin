  ...
  <PageWrapper contentBackground contentClass="flex" dense contentFullHeight fixedHeight>
    <div style="width: 80%">
      <BasicTable :loading="loading" :dataSource="items" @register="registerTable">
        ...
        <template #toolbar>
          <a-button type="primary" @click="handleAdd">添加用地规划</a-button>
        </template>
      </BasicTable>
    </div>
  </PageWrapper>
    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.contentManagerLandPlanningListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.common_land_planning_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.contentManagerShopProductRemoveGQL,
            variables: {
              id: record.id,
            },
          })
