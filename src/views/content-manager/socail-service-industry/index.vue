<template>
  <PageWrapper contentBackground contentClass="flex" dense contentFullHeight fixedHeight>
    <BasicTable :loading="loading" :dataSource="items" @register="registerTable">
      ...
      <template #toolbar>
        <a-button type="primary" @click="handleAdd">添加社会化服务</a-button>
      </template>
    </BasicTable>
  </PageWrapper>
</template>
    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.contentManagerSocialServiceIndustryListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.big_data_social_service_industry_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.contentManagerSocialServiceIndustryRemoveGQL,
            variables: {
              id: record.id,
            },
          })
      ...
      const handleEdit = (record: Recordable) => {
        instance?.proxy?.$router.push({
          path: '/content-manager/socail-service-industry/edit',
          query: { id: record.id },
        });
      };
      const handleAdd = () => {
