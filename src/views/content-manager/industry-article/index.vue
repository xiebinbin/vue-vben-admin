    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.contentManagerIndustryArticleListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.big_data_industry_article_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.contentManagerIndustryArticleRemoveGQL,
            variables: {
              id: record.id,
            },
          })
