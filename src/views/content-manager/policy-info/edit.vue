...
<script lang="ts">
  ...
  const pageTitle = ref<string>('政策资讯管理');
      ...
      if (!!itemId.value) {
        ...
        onMounted(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.contentManagerPolicyInfoGQL,
            variables: {
              id: itemId.value,
            },
          });
          const info = data?.data?.big_data_policy_article;
          methods.setFieldsValue({
            title: info.title,
            desc: info.desc,
            sort: info.sort,
        ...
        pageTitle.value = '编辑政策资讯';
      } else {
        pageTitle.value = '新增政策资讯';
      }
      return {
        schemas,
        handleReset,
        handleSubmit: () => {
          if (!!itemId.value) {
            ...
            instance?.proxy?.$apollo
              ?.mutate({
                mutation: gql.contentManagerPolicyInfoUpdateGQL,
                variables: {
                  id: itemId.value,
                  input: formData,
                },
            ...
            instance?.proxy?.$apollo
              ?.mutate({
                mutation: gql.contentManagerPolicyInfoAddGQL,
                variables: {
                  input: formData,
                },
              })
