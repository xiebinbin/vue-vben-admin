<template>
  <PageWrapper contentBackground contentClass="flex" dense contentFullHeight fixedHeight>
    <BasicTable :loading="loading" :dataSource="items" @register="registerTable">
      ...
      <template #toolbar>
        <a-button type="primary" @click="handleAdd">物资出库</a-button>
      </template>
    </BasicTable>
  </PageWrapper>
</template>
    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.traceabilityMaterialOutputListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.traceability_material_output_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.traceabilityMaterialOutputRemoveGQL,
            variables: {
              id: record.id,
            },
          })
      ...
      const handleAdd = () => {
        instance?.proxy?.$router.push({
          path: '/traceability/material/output/add',
        });
      };
      return {
        registerTable,
