<template>
  <PageWrapper contentBackground contentClass="flex" dense contentFullHeight fixedHeight>
    <BasicTable :loading="loading" :dataSource="items" @register="registerTable">
      ...
      <template #toolbar>
        <a-button type="primary" @click="handleAdd">产品出库</a-button>
      </template>
    </BasicTable>
  </PageWrapper>
</template>
    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.traceabilityProductOutputListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.traceability_product_output_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.traceabilityProductOutputRemoveGQL,
            variables: {
              id: record.id,
            },
          })
      ...
      const handleAdd = () => {
        instance?.proxy?.$router.push({
          path: '/traceability/product/output/add',
        });
      };
      return {
        registerTable,
