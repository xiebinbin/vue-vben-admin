    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.traceabilitySaleProductListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.traceability_sale_product_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.traceabilitySaleProductRemoveGQL,
            variables: {
              id: record.id,
            },
          })
      ...
      const handleEdit = (record: Recordable) => {
        instance?.proxy?.$router.push({
          path: '/traceability/sale-product/edit',
          query: { id: record.id },
        });
      };
      const handleAdd = () => {
