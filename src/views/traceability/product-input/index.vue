<template>
  <PageWrapper contentBackground contentClass="flex" dense contentFullHeight fixedHeight>
    <BasicTable :loading="loading" :dataSource="items" @register="registerTable">
      ...
      <template #toolbar>
        <a-button type="primary" @click="handleAdd">产品入库</a-button>
      </template>
    </BasicTable>
  </PageWrapper>
</template>
    ...
    setup() {
      ...
      const loadData = async (page: number) => {
        ...
        instance?.proxy?.$nextTick(async () => {
          const data = await instance?.proxy?.$apollo?.query({
            query: gql.traceabilityProductInputListGQL,
            variables: {
              input: {
                ...query,
                page,
          ...
          const listData = data?.data.traceability_product_input_list;
      ...
      const handleDelete = (record: Recordable) => {
        ...
        instance?.proxy?.$apollo
          ?.mutate({
            mutation: gql.traceabilityProductInputRemoveGQL,
            variables: {
              id: record.id,
            },
          })
      ...
      const handleEdit = (record: Recordable) => {
        instance?.proxy?.$router.push({
          path: '/traceability/product/input/edit',
          query: { id: record.id },
        });
      };
      const handleAdd = () => {
