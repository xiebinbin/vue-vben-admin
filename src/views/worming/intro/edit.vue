...
<script lang="ts">
  ...
  const pageTitle = ref<string>('性诱测报仪介绍');
    ...
    setup() {
      ...
      onMounted(async () => {
        const data = await instance?.proxy?.$apollo?.query({
          ...
          variables: {
            keys: ['worming-intro', 'worming-intro-image'],
          },
        });
        ...
        formData.image = keyVals['worming-intro-image'] ?? '';
        methods.setFieldsValue({
          intro: keyVals['worming-intro'] ?? '',
          image: keyVals['worming-intro-image'] == '' ? [] : [keyVals['worming-intro-image']],
        });
        formData.intro = keyVals['worming-intro'] ?? '';
      });
      return {
        schemas,
        handleReset,
