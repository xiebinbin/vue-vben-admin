...
<script lang="ts">
  ...
  const pageTitle = ref<string>('水肥一体介绍');
    ...
    setup() {
      ...
      onMounted(async () => {
        const data = await instance?.proxy?.$apollo?.query({
          ...
          variables: {
            keys: ['shuifei-intro', 'shuifei-intro-image'],
          },
        });
        ...
        formData.image = keyVals['shuifei-intro-image'] ?? '';
        methods.setFieldsValue({
          intro: keyVals['shuifei-intro'] ?? '',
          image: keyVals['shuifei-intro-image'] == '' ? [] : [keyVals['shuifei-intro-image']],
        });
        formData.intro = keyVals['shuifei-intro'] ?? '';
      });
      return {
        schemas,
        handleReset,
